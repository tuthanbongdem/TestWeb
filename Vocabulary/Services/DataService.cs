...
public class DataService
{
    ...
    public async Task UpdateWordFullAsync(Word w)
    {
        await _supa.UpdateWordAsync(w);
        var idx = _words.FindIndex(x => x.Id == w.Id);
        if (idx >= 0) _words[idx] = w;
        Notify();
    }

    public bool WordExists(string eng, int excludeId = 0) =>
        _words.Any(w => w.Id != excludeId && w.English.Equals(eng.Trim(), StringComparison.OrdinalIgnoreCase));
    ...
    public async Task UpdatePhraseFullAsync(Phrase p)
    {
        await _supa.UpdatePhraseAsync(p);
        var idx = _phrases.FindIndex(x => x.Id == p.Id);
        if (idx >= 0) _phrases[idx] = p;
        Notify();
    }

    public bool PhraseExists(string eng, int excludeId = 0) =>
        _phrases.Any(p => p.Id != excludeId && p.English.Equals(eng.Trim(), StringComparison.OrdinalIgnoreCase));
